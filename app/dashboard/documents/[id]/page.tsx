    ...
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 p-4 lg:p-10 print:p-0 print:bg-white transition-colors">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {order.status !== 'završen' && (
            <form action={async () => {
                ...
                <button type="submit" className="flex items-center gap-2 px-6 py-2 bg-green-600 text-white font-bold text-xs uppercase shadow-md hover:bg-green-700 transition-all active:scale-95">
      ...
      <div className="max-w-[210mm] min-h-[297mm] mx-auto bg-white text-black p-[15mm] shadow-2xl print:shadow-none print:p-0 print:m-0 flex flex-col font-sans border border-slate-200 print:border-none relative overflow-hidden">
        ...
        <div className="text-center mb-10">
            ...
            <p className="text-[10px] uppercase font-black mt-1 tracking-[0.4em] text-slate-400 print:text-black">auto-mehaničarska usluga</p>
        </div>

        {/* INFO GRID */}
        <div className="grid grid-cols-4 border border-black text-[10px] mb-8 divide-x divide-black bg-slate-50/30">
            <div className="p-2"><p className="text-slate-400 font-bold mb-1 uppercase text-[7px]">Broj dokumenta</p><p className="font-bold text-sm">{order.number}</p></div>
            <div className="p-2"><p className="text-slate-400 font-bold mb-1 uppercase text-[7px]">Datum otvaranja</p><p className="font-bold text-sm">{order.dateEntry?.toLocaleDateString('sr-RS')}</p></div>
            <div className="p-2"><p className="text-slate-400 font-bold mb-1 uppercase text-[7px]">Status</p><p className="font-black uppercase text-blue-600 text-sm">{order.status}</p></div>
            <div className="p-2"><p className="text-slate-400 font-bold mb-1 uppercase text-[7px]">Administrator</p><p className="font-bold text-sm">Sistem</p></div>
        </div>

        {/* NARUČILAC I VOZILO */}
        <div className="grid grid-cols-2 border border-black mb-10 divide-x divide-black">
            <div className="p-5 space-y-1">
                <p className="text-[8px] font-black uppercase text-slate-400 mb-1 tracking-widest">Naručilac radova</p>
            ...
            <div className="p-5 bg-slate-50/30">
                <p className="text-[8px] font-black uppercase text-slate-400 mb-2 tracking-widest">Identifikacija vozila</p>
                <div className="grid grid-cols-2 gap-4 text-[10px]">
                    <div><span className="text-slate-400 font-bold uppercase text-[7px]">Vozilo:</span><p className="font-black uppercase text-sm">{vehicle.make} {vehicle.model}</p></div>
                    <div><span className="text-slate-400 font-bold uppercase text-[7px]">Registracija:</span><p className="font-black uppercase underline underline-offset-4 text-sm">{vehicle.plateNumber}</p></div>
                    <div className="col-span-2 mt-2">
                        <span className="text-slate-400 font-bold uppercase text-[7px]">Tehnički detalji (VIN | Snaga | Zapremina | Gorivo)</span>
                        <p className="font-mono font-bold tracking-tight text-[11px] mt-1 border-t border-slate-200 pt-1">
                        {vehicle.vin} | {vehicle.power} | {vehicle.displacement} ccm | {vehicle.fuelType}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div className="mb-6">
            <div className="border border-black">
                <div className="bg-slate-800 text-white p-2 text-[9px] font-black uppercase tracking-widest">I - Rezervni delovi i materijal</div>
                <table className="w-full text-[10px] border-collapse">
                    <thead>
                        <tr className="border-b border-black text-left bg-slate-200">
                            ...
                            <th className="p-2 border-r border-black">Naziv / Opis stavke</th>
                    ...
                    <tfoot className="bg-slate-100 border-t border-black">
                      <tr className="font-black uppercase text-[9px]">
                        <td colSpan={4} className="p-2 border-r border-black text-right">Ukupno delovi (Neto):</td>
                        <td className="p-2 text-right font-mono text-xs">{sumParts.toLocaleString('sr-RS', { minimumFractionDigits: 2 })}</td>
                      </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <div className="mb-10">
            <div className="border border-black">
                <div className="bg-slate-800 text-white p-2 text-[9px] font-black uppercase tracking-widest">II - Servisne usluge i rad</div>
                <table className="w-full text-[10px] border-collapse">
                    <thead>
                        <tr className="border-b border-black text-left bg-slate-200">
                            <th className="p-2 border-r border-black w-10 text-center">RB</th>
                            <th className="p-2 border-r border-black">Opis servisne usluge</th>
                            <th className="p-2 border-r border-black w-14 text-center">Kol.</th>
                            <th className="p-2 border-r border-black w-28 text-right">Cena</th>
                            <th className="p-2 w-32 text-right">Vrednost</th>
                        </tr>
                    ...
                    <tfoot className="bg-slate-100 border-t border-black">
                      <tr className="font-black uppercase text-[9px]">
                        <td colSpan={4} className="p-2 border-r border-black text-right">Ukupno rad (Neto):</td>
                        <td className="p-2 text-right font-mono text-xs">{sumServices.toLocaleString('sr-RS', { minimumFractionDigits: 2 })}</td>
                      </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <div className="mt-auto border-t-2 border-black pt-6">
            <div className="flex justify-between items-start mb-16">
                <div className="text-[7.5px] max-w-[50%] leading-tight text-slate-700 italic pr-10 space-y-1 print:text-black">
                    ...
                    <p>Naručilac je saglasan: 1.) Da se izvrše navedeni radovi. 2.) Da se izvrše i obave nepredvidivi radovi koji su neophodni za izvršenje naručenih radova.</p>
                    <p>3.) Da se izvršeni radovi naplate po važećim cenama servisa. 4.) Da rok završetka radova može biti produžen usled nedostatka delova na tržištu.</p>
                </div>
                ...
                <div className="w-[60mm] space-y-1 bg-slate-50 p-3 border border-black shadow-sm">
                  ...
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-[12px] font-black uppercase italic tracking-tighter">UKUPNO:</span>
                    <div className="text-right">
                      <p className="text-2xl font-mono tracking-tighter leading-none">
                        {grossAmount.toLocaleString('sr-RS', { minimumFractionDigits: 2 })} 
                        <span className="text-[9px] not-italic ml-1 font-bold">RSD</span>
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <div className="hidden print:grid grid-cols-3 gap-20 text-center mt-10">
                <div className="border-t border-black pt-2"><p className="text-[8px] font-black uppercase tracking-widest">Odgovorno lice</p></div>
                <div className="border-t border-black pt-2"><p className="text-[8px] font-black uppercase tracking-widest">Naručilac radova (potpis)</p></div>
                <div className="border-t border-black pt-2"><p className="text-[8px] font-black uppercase tracking-widest">Vozilo preuzeo</p></div>
            </div>
        </div>
      </div>
    </div>
